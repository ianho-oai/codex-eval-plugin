...
| Approve tasks | Review three Basic and five Hard tasks per workflow, plain-language summaries, acceptance checks, and source links. | The skill adapts examples from the local benchmark catalog or explains an original design. Tasks are freshly authored, not official benchmark reproductions. |
...
- One iteration per task/model/effort first. After reviewing results and costs, the agent asks before two additional rounds for consistency and averaging.
...
Both axis dropdowns keep the same options for every provider. New views start with task cost versus latency; saved selections persist. Average score and cost per verified success are available on either axis and force grouped mode. Task cost displays Copilot native dollar usage valuation with an explicit non-invoice label; missing measurements remain visible.

The chart comes first, with provider/model and difficulty/task checkbox groups, select-all controls, toggleable model labels, and independent logarithmic axes. Hover details show model/effort, task, difficulty, pass count, cost, and end-to-end latency. The task table uses short workflow descriptions. In Median focus, choose model × effort or model grouping, with providers kept separate. Ordinary metrics use medians of measured task/configuration averages; score uses passed/completed runs and cost per verified success uses total completed cost divided by passes. The top-right Median focus switch fades task points and emphasizes these diamonds; hover shows sample coverage and the selected-axis medians.
