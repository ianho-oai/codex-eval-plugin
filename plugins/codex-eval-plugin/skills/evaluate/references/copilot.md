...
Get approval for the exact small smoke scope, unless already authorized. Each command below makes paid inference calls on one bundled edit-and-test fixture, one repeat, no agent or grader timeout, no credit cap by default. The independent grader verifies the edited candidate in the actual launch context. The customer-run readiness gate separately requires a successful native shell-test receipt. Use a new output directory for every attempt.
...
The axis dropdown options stay fixed across providers. New views default to task cost versus latency; saved or user-selected axes are preserved without automatic provider-based changes. Task cost displays Copilot’s native dollar usage value through a display-only projection alongside other providers’ costs. Tooltips and the chart note identify usage value as distinct from invoice cost. Both axis dropdowns use the same fixed options, including average score, without separate Copilot credit/value options. Unknown usage stays missing. Reports and CSV retain these fields; summary known totals disclose missing receipts. An explicit USD spend stop pauses on missing currency cost and cannot enforce a Copilot credit ceiling. GitHub's own billing controls remain separate. Timeouts and numeric credit limits can still be exceeded by in-flight work; no credit cap does not mean free usage or unlimited plan entitlement.
