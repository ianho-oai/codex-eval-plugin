...
Do not generate dashboards or ad hoc orchestration after approval. Do not install additional plugins/skills/connectors. If a key, model, dependency, or permitted endpoint is missing, preserve the failure and report the specific blocker. Never route around host policy. Unknown cost stops future calls; active calls drain and a stop threshold can overshoot by the cost of all in-flight calls and is not a provider billing cap.
