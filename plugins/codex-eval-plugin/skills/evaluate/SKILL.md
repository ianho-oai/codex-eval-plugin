...
Include every supported reasoning/effort level for each selected model by default, using the dated catalog and installed headless CLI capabilities. Recheck capability sources when refreshing models; do not invent a common level list or include modes that change the agent harness (for example, automatic delegation). Explain exclusions and never silently drop failed levels. Use `EVAL configure SUITE --all-efforts` before freezing the plan. Customers may request a narrower sweep with repeated `--effort` flags. Use three repeats per task/model/effort configuration by default; a requested quick sweep can use `--repeats 1`. Explain that charts show arithmetic means for measured cost, latency, and tokens across those repeats, including failed attempts; raw attempts remain available. Change repeats only when the customer explicitly requests it.
