...
For each workflow, propose easy, medium, and hard tasks that the selected models are expected to complete successfully. My primary comparison is how much time, tokens, and cost they need for a correct result. Provide clear requirements, enough context, and reasonable time limits; increase coding complexity across tiers while keeping setup simple. Prepare the environment before timed execution and retain genuine failures in the results. Include a short, human-readable workflow description, objective pass/fail checks, and relevant inspiration from the plugin's benchmark catalog with original source links. Create original tasks when no suitable example fits. Ask me to approve the task proposal.
...
Audit each grader against the visible requirements before paid runs. Check an independently written valid alternative as well as the oracle and broken examples. During execution, use the bundled reflect command to inspect early failures: any infrastructure error, or at least 50% failures after three scorable attempts for a task, should trigger a review. Investigate setup, unclear requirements, overstrict grading, and unrealistic scope before concluding that models cannot solve the task. Pause affected work when needed. If a test is defective, preserve its original results and costs, prepare a corrected revision, show me the concrete change for approval, then rerun all affected configurations fairly. Keep genuine coding failures; do not weaken valid checks or retry until everything passes. Limit corrective revisions to two per task before discussing a redesign. Review remaining failures at the end too.
