...
## Copilot: separate account, permission and model checks

Follow the [Copilot setup guide](../plugins/codex-eval-plugin/skills/evaluate/references/copilot.md). `gh auth login`, browser account changes, and reinstalling Copilot do not establish the selected Copilot identity. Use `copilot login --web-flow`, verify `/user`, then select `--copilot-account LOGIN` explicitly. That mode ignores environment tokens. For unattended token mode, use a personal fine-grained PAT with Copilot Requests and place it in ignored `.env.local` as `COPILOT_GITHUB_TOKEN`; exported variables override that file. Classic PATs are unsupported.

A policy-denied message can arise from account, token, plan/model access, or organization policy. It does not prove an individual account is an enterprise account. Check the exact binary/version and `/model`; run an approved model/effort smoke from the intended launch context. Copilot doctor does not query an account model list. No automatic account switch, BYOK fallback, model substitution, or retry of genuine grading failures occurs.
